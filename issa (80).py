...
HEURES_LIST = [
    "08h-09h", "09h-10h", "10h-11h", "11h00-11h30", "11h30-12h",
...
def ajouter_entete_senegal_officiel(pdf, titre_document=""):
    ...
    if hasattr(pdf, "set_line_width"): pdf.set_line_width(0.8)
    elif hasattr(pdf, "set_linewidth"): pdf.set_linewidth(0.8)
...
def calculer_bulletin_eleve(classe, eleve, periode):
    ...
    if not notes_df.empty and "Classe" in notes_df.columns:
        ...
        elif "Periode" in notes_df.columns: cond_per = notes_df["Periode"] == periode
        elif "Période" in notes_df.columns: cond_per = notes_df["Période"] == periode
        else: cond_per = True
    ...
    lignes_bulletin = []
    total_points_global, total_coefficients_global, total_bareme_global = 0.0, 0.0, 0.0
    ...
    for mat in liste_matieres:
        ...
        if is_elem:
            ...
            lignes_bulletin.append({
                "Matiere": mat, "Bareme": bareme_m, "Composition": comp,
            ...
            lignes_bulletin.append({
                "Matiere": mat, "Coefficient": coef, "Devoir1": d1, "Devoir2": d2,
                "Composition": comp, "MoyenneMatiere": round(moy_matiere, 2),
        ...
        for mat in liste_matieres:
            ...
            if not n_m.empty:
                ...
                if is_elem:
                    ...
                    pts += m_mat * coef
                    coefs += coef
        if is_elem: moyennes_classe[el] = round((pts / bareme_tot_el) * 10.0, 2) if bareme_tot_el > 0 else 0.0
        else: moyennes_classe[el] = round(pts / coefs, 2) if coefs > 0 else 0.0
    ...
    if not vs_df.empty and "Classe" in vs_df.columns and "Eleve" in vs_df.columns:
        cond_vs = (vs_df["Classe"] == classe) & (vs_df["Eleve"] == eleve)
        if "Periode" in vs_df.columns: vs_row = vs_df[cond_vs & (vs_df["Periode"] == periode)]
        elif "Période" in vs_df.columns: vs_row = vs_df[cond_vs & (vs_df["Période"] == periode)]

    abs_just = int(vs_row.iloc[0]["AbsencesJustifiees"]) if not vs_row.empty and "AbsencesJustifiees" in vs_row.columns and pd.notna(vs_row.iloc[0]["AbsencesJustifiees"]) else 0
    abs_non_just = int(vs_row.iloc[0]["AbsencesNonJustifiees"]) if not vs_row.empty and "AbsencesNonJustifiees" in vs_row.columns and pd.notna(vs_row.iloc[0]["AbsencesNonJustifiees"]) else 0
    retards = int(vs_row.iloc[0]["Retards"]) if not vs_row.empty and "Retards" in vs_row.columns and pd.notna(vs_row.iloc[0]["Retards"]) else 0
    heures_p = int(vs_row.iloc[0]["HeuresPerdues"]) if not vs_row.empty and "HeuresPerdues" in vs_row.columns and pd.notna(vs_row.iloc[0]["HeuresPerdues"]) else 0
    obs = str(vs_row.iloc[0]["Observations"]) if not vs_row.empty and "Observations" in vs_row.columns and pd.notna(vs_row.iloc[0]["Observations"]) else "RAS"
    decision = str(vs_row.iloc[0]["DecisionConseil"]) if not vs_row.empty and "DecisionConseil" in vs_row.columns and pd.notna(vs_row.iloc[0]["DecisionConseil"]) else "Encouragements"
...
def generer_pdf_bulletin(bul_data):
    ...
    except Exception: font_family = "Arial"
    ...
    try:
        ...
        if isinstance(output_pdf, str): return output_pdf.encode('latin1', 'replace')
        elif isinstance(output_pdf, (bytes, bytearray)): return bytes(output_pdf)
        else: return bytes(pdf.output())
...
def generer_zip_bulletins_classe(classe, periode):
    ...
    eleves = eleves_df[eleves_df["Classe"] == classe] if "Classe" in eleves_df.columns else pd.DataFrame()
...
def generer_pdf_liste_eleves_classe(classe):
    if "eleves_db" not in st.session_state or st.session_state.eleves_db.empty:
        ...
        df_eleves = st.session_state.eleves_db[st.session_state.eleves_db["Classe"] == classe] if "Classe" in st.session_state.eleves_db.columns else pd.DataFrame(columns=["Nom Complet", "Classe", "Date de Naissance"])
    ...
    for i, h in enumerate(headers): pdf.cell(col_widths[i], 7, nettoyer_texte_pdf(h), 1, 0, "C", True)
    pdf.ln()
...
def generer_pdf_edt(classe, df_edt):
    ...
    for col in df_edt.columns: pdf.cell(col_w, 7, nettoyer_texte_pdf(str(col)[:8]), 1, 0, "C", True)
    pdf.ln()
...
# ==========================================
# 6. ESPACE PROFESSEURS / MAÎTRES
# ==========================================
elif st.session_state.espace_actif == "👨‍🏫 Espace Professeurs / Maîtres":
    st.markdown('<div style="color: #0F172A; font-size: 2.2rem; font-weight: 900;">Espace Enseignants & Saisie Pédagogique Locale / Supabase</div>', unsafe_allow_html=True)
    ...
    if not st.session_state.prof_logged:
            ...
            if btn_p_login:
                ...
                for target_df in targets:
                    for _, row in target_df.iterrows():
                        ...
                        name_match = (
                            input_val_norm == db_nom_norm or input_val_norm == db_prenom_norm or
                            input_val_norm == full_name_1 or input_val_norm == full_name_2 or
                            db_nom_norm in input_val_norm or db_prenom_norm in input_val_norm or
            ...
            if not periodes_possibles:
                ...
                if eleves_list:
                    ...
                    for el in eleves_list:
                        ...
                        if is_elem_prof:
                            rows_notes.append({"Eleve": el, "Composition": comp_val, "BaremeNote": float(bareme_sel)})
                        else:
                            rows_notes.append({"Eleve": el, "Devoir1": d1_val, "Devoir2": d2_val, "Composition": comp_val, "BaremeNote": float(bareme_sel)})
        ...
        with t_taf_prof:
            ...
            st.info(f"Les travaux assignés ici pour la classe de **{classe_autorisee}** sont enregistrés dans la base de données.")
            ...
            with st.form("form_taf_prof", clear_on_submit=True):
                ...
                btn_publier_taf = st.form_submit_button("🚀 Publier et Enregistrer dans Supabase")
                ...
                if btn_publier_taf:
                    if titre_taf and consignes_taf:
                        ...
                        st.success("✅ Travail à faire publié et sauvegardé dans Supabase !")
                        st.rerun()
                    else:
            ...
            if not df_taf_cls.empty:
                edited_taf = st.data_editor(df_taf_cls, num_rows="dynamic", use_container_width=True, key="editor_taf_prof")
                if st.button("💾 Mettre à jour les devoirs", key="btn_save_taf_edited"):
                    st.session_state.travail_a_faire_db = st.session_state.travail_a_faire_db[st.session_state.travail_a_faire_db["Classe"] != classe_autorisee]
                    st.session_state.travail_a_faire_db = pd.concat([st.session_state.travail_a_faire_db, edited_taf], ignore_index=True)
        ...
        with t_appel:
            st.markdown("### 📋 Registre d'Appel & Absences")
            date_appel = st.date_input("Date du jour", value=datetime.today(), key="appel_date")
            df_el_app = pd.DataFrame()
            if "eleves_db" in st.session_state and "Classe" in st.session_state.eleves_db.columns:
                df_el_app = trier_eleves_par_nom(st.session_state.eleves_db[st.session_state.eleves_db["Classe"] == classe_autorisee])

            if not df_el_app.empty:
                rows_app = []
                for _, r_e in df_el_app.iterrows():
                    nom_el = str(r_e.get("Nom Complet", ""))
                    rows_app.append({"Élève": nom_el, "Statut": "Présent", "Motif": ""})
                df_appel_init = pd.DataFrame(rows_app)
                edited_appel = st.data_editor(df_appel_init, num_rows="fixed", use_container_width=True, key="editor_appel_prof")

                if st.button("💾 Valider la Feuille d'Appel", key="btn_save_appel"):
                    edited_appel["Date"] = str(date_appel)
                    edited_appel["Classe"] = classe_autorisee
                    
                    st.session_state.absences_db = pd.concat([st.session_state.absences_db, edited_appel[edited_appel["Statut"] != "Présent"]], ignore_index=True)
                    ...
                    enregistrer_log_action(prof_connecte, "APPEL", f"Appel enregistré pour {classe_autorisee} le {date_appel}")
                    st.success("✅ Registre d'appel enregistré dans Supabase avec succès !")
            else:
                st.warning("⚠️ Aucun élève trouvé pour cette classe.")

        with t_cond:
            st.markdown("### ⚠️ Conduite, Discipline & Conseil de Classe")
            periodes_vs = obtenir_periodes_pour_classe(classe_autorisee)
            p_vs_sel = st.selectbox("Période concernée", periodes_vs, key="vs_p_sel")
            df_el_vs = trier_eleves_par_nom(st.session_state.eleves_db[st.session_state.eleves_db["Classe"] == classe_autorisee]) if "eleves_db" in st.session_state and "Classe" in st.session_state.eleves_db.columns else pd.DataFrame()

            if not df_el_vs.empty:
                rows_vs = []
                for _, r_e in df_el_vs.iterrows():
                    nom_el = str(r_e.get("Nom Complet", ""))
                    rows_vs.append({
                        "Eleve": nom_el, "AbsencesJustifiees": 0, "AbsencesNonJustifiees": 0,
                        "Retards": 0, "HeuresPerdues": 0, "Observations": "Très bon travail",
                        "DecisionConseil": "Encouragements"
                    })
                edited_vs = st.data_editor(pd.DataFrame(rows_vs), num_rows="dynamic", use_container_width=True, key="editor_vs_prof")

                if st.button("💾 Sauvegarder la Vie Scolaire", key="btn_save_vs"):
                    edited_vs["Classe"] = classe_autorisee
                    edited_vs["Periode"] = p_vs_sel
                    edited_vs["Période"] = p_vs_sel
                    
                    if "viescolaire_db" in st.session_state and not st.session_state.viescolaire_db.empty:
                        cond_old = (st.session_state.viescolaire_db["Classe"] == classe_autorisee) & ((st.session_state.viescolaire_db.get("Periode") == p_vs_sel) | (st.session_state.viescolaire_db.get("Période") == p_vs_sel))
                        st.session_state.viescolaire_db = st.session_state.viescolaire_db[~cond_old]
                    ...
                    st.success("✅ Vie Scolaire mise à jour dans Supabase !")
            else:
                st.warning("⚠️ Aucun élève trouvé pour cette classe.")

        with t_cahier:
            st.markdown("### 📑 Cahier de Texte National")
            with st.form("form_ct_prof", clear_on_submit=True):
                c_d1, c_d2 = st.columns(2)
                with c_d1:
                    d_ct = st.date_input("Date de la séance", value=datetime.today())
                    m_ct = st.selectbox("Matière", [matiere_principale] + [m for m in st.session_state.matieres_def["Matière"].unique() if m != matiere_principale])
                with c_d2:
                    cont_ct = st.text_area("Contenu du cours dispensé")
                    taf_ct = st.text_area("Travail à faire pour la prochaine séance")

                if st.form_submit_button(" Enregistrer au Cahier de Texte"):
                    new_ct = {
                        "Professeur": prof_connecte, "Date": str(d_ct), "Classe": classe_autorisee,
                        "Matière": m_ct, "Contenu": cont_ct, "Travail à faire": taf_ct
                    }
                    ...
                    st.success("✅ Cahier de texte sauvegardé avec succès dans Supabase !")

            st.markdown("---")
            if not st.session_state.cahier_textes.empty:
                st.dataframe(st.session_state.cahier_textes[st.session_state.cahier_textes["Classe"] == classe_autorisee], use_container_width=True)

        with t_edt_prof:
            st.markdown(f"### 📅 Emploi du Temps Officiel - Classe : {classe_autorisee}")
            grid_prof = get_or_create_edt(classe_autorisee)
            edited_grid_prof = st.data_editor(grid_prof, use_container_width=True, key=f"edt_prof_{classe_autorisee}")

            if st.button("💾 Enregistrer Emploi du Temps", key="btn_save_edt_prof"):
                st.session_state.edt_grid_db[classe_autorisee] = edited_grid_prof
                rows_to_save = []
                for cls_k, df_k in st.session_state.edt_grid_db.items():
                    for jour_k in df_k.index:
                        for heure_k in df_k.columns:
                            val_k = df_k.loc[jour_k, heure_k]
                            if val_k:
                                rows_to_save.append({"classe": cls_k, "jour": jour_k, "heure": heure_k, "valeur": val_k})
                df_edt_save = pd.DataFrame(rows_to_save)
                save_df_to_db(df_edt_save, "edt_grid")
                st.success("✅ Emploi du temps enregistré avec succès !")

# ==========================================
# 7. ESPACE PARENTS / ÉLÈVES (COMPLÈTEMENT DYNAMIQUE)
# ==========================================
elif st.session_state.espace_actif == "👨‍👩‍👧 Espace Parents / Élèves":
    st.markdown('<div style="color: #0F172A; font-size: 2.2rem; font-weight: 900;">👨‍👩‍👧 Portail d\'Accès Parents & Élèves</div>', unsafe_allow_html=True)
    ...
    if not st.session_state.parent_logged:
        st.info("🔒 Authentification Parent : Veuillez saisir le numéro de téléphone enregistré ou sélectionner votre enfant.")
        with st.form("form_login_parent"):
            col_p1, col_p2 = st.columns(2)
            with col_p1:
                p_tel = st.text_input("Téléphone du parent / tuteur", placeholder="Ex: 771234567")
            with col_p2:
                eleves_all = st.session_state.eleves_db["Nom Complet"].tolist() if "eleves_db" in st.session_state and not st.session_state.eleves_db.empty and "Nom Complet" in st.session_state.eleves_db.columns else ["Exemple Élève"]
                p_child = st.selectbox("Sélectionner l'Élève", sorted(list(set(eleves_all))))

            btn_login_parent = st.form_submit_button("🔑 Connexion Espace Parent")
            ...
            if btn_login_parent:
                ...
                if "eleves_db" in st.session_state and not st.session_state.eleves_db.empty and "Nom Complet" in st.session_state.eleves_db.columns:
                    r_child = st.session_state.eleves_db[st.session_state.eleves_db["Nom Complet"] == p_child]
                    if not r_child.empty and "Classe" in r_child.columns:
                        cls_found = str(r_child.iloc[0]["Classe"])

                st.session_state.parent_logged = True
                st.session_state.parent_eleve_sel = p_child
                st.session_state.parent_classe_sel = cls_found
                enregistrer_log_action(f"Parent ({p_child})", "CONNEXION_PARENT", f"Accès au profil de {p_child} - Classe {cls_found}")
                st.success("Connexion réussie !")
                st.rerun()
    else:
        enfant_nom = st.session_state.parent_eleve_sel
        enfant_classe = st.session_state.parent_classe_sel

        st.markdown(
            f"""
            <div style="background: linear-gradient(135deg, #FFFFFF 0%, #F0F9FF 100%); padding: 22px; border-radius: 20px; border: 2px solid #0EA5E9; margin-bottom: 25px; display: flex; justify-content: space-between; align-items: center; box-shadow: 0 8px 25px rgba(14,165,233,0.15);">
                <div>
                    <h3 style="margin:0; color: #0F172A; font-weight: 800;">Élève : {enfant_nom}</h3>
                    <p style="margin: 5px 0 0 0; color: #0EA5E9; font-weight: 700; font-size: 1.1rem;">Classe : {enfant_classe} | Suivi Pédagogique en Temps Réel</p>
                </div>
            </div>
            """,
            unsafe_allow_html=True
        ...
        tp_taf, tp_bulletin, tp_edt, tp_ct, tp_vs, tp_msg = st.tabs([
            "📌 Devoirs & Travaux à Faire",
            "📊 Bulletin de Notes PDF",
            "📅 Emploi du Temps",
            "📑 Cahier de Texte",
            "⚠️ Vie Scolaire & Assiduité",
            "💬 Messages & Échanges avec l'École"
        ])

        # TAB 1 : TRAVAUX À FAIRE
        with tp_taf:
            st.markdown(f"### 📌 Travaux & Devoirs à Faire - Classe : {enfant_classe}")
            df_taf_p = pd.DataFrame()
            if "travail_a_faire_db" in st.session_state and not st.session_state.travail_a_faire_db.empty and "Classe" in st.session_state.travail_a_faire_db.columns:
                df_taf_p = st.session_state.travail_a_faire_db[st.session_state.travail_a_faire_db["Classe"] == enfant_classe]
            ...
            if not df_taf_p.empty:
                for _, row in df_taf_p.iterrows():
                    st.markdown(
                        f"""
                        <div class="work-card">
                            <h4 style="color: #0EA5E9; margin: 0 0 10px 0; font-weight: 800;">📘 {row.get('Titre', 'Devoir')} ({row.get('Matière', 'Général')})</h4>
                            <p style="margin: 4px 0; font-size: 0.95rem;"><b>Enseignant :</b> {row.get('Professeur', 'N/A')} | <b>Date de Rendu :</b> <span style="color: #E11D48; font-weight: 700;">{row.get('DateRendu', 'N/A')}</span></p>
                            <p style="margin: 10px 0; font-size: 1rem; color: #334155;"><b>Consignes :</b><br>{row.get('Consignes', 'Aucune consigne.')}</p>
                        </div>
                        """,
                        unsafe_allow_html=True
                    )
                    ...
                    with c_link1:
                        if row.get("LienUrl") and str(row.get("LienUrl")).startswith("http"):
                            st.markdown(f"🔗 [Accéder à la ressource externe]({row.get('LienUrl')})")
                    with c_link2:
                        if row.get("LienVideo") and str(row.get("LienVideo")).startswith("http"):
                        ...
                        if row.get("FichierB64") and row.get("FichierNom"):
                            try:
                                ...
                                st.download_button(
                                    ...
                                    data=f_bytes,
                                    file_name=str(row.get('FichierNom')),
                                    mime=str(row.get('FichierType', 'application/octet-stream')),
                                    key=f"dl_taf_{row.get('ID')}"
                                )
                            except Exception:
                                pass
                    st.markdown("---")
            else:
                st.info("🎉 Aucun devoir à faire pour le moment dans cette classe.")

        # TAB 2 : BULLETIN DE NOTES
        with tp_bulletin:
            st.markdown(f"### 📊 Bulletins de Notes Officiels - {enfant_nom}")
            periodes_p = obtenir_periodes_pour_classe(enfant_classe)
            if periodes_p:
                p_sel_bul = st.selectbox("Sélectionner la Période", periodes_p, key="p_sel_bul_parent")
                b_data = calculer_bulletin_eleve(enfant_classe, enfant_nom, p_sel_bul)

                st.markdown(
                    f"""
                    <div style="background: #FFFFFF; padding: 20px; border-radius: 18px; border: 2px solid #BAE6FD; margin-bottom: 20px;">
                        <h4 style="margin: 0 0 10px 0; color: #0F172A;">Moyenne Générale : <span style="color: #0EA5E9; font-weight: 900; font-size: 1.5rem;">{b_data['moyenne_generale']} / {b_data['total_bareme']}</span></h4>
                        <p style="margin: 0; font-weight: 700; color: #475569;">Rang : {b_data['rang']} | Total Points : {b_data['total_points']}</p>
                    </div>
                    """,
                    unsafe_allow_html=True
                )
                ...
                if b_data["lignes"]:
                    st.dataframe(pd.DataFrame(b_data["lignes"]), use_container_width=True)

                pdf_b_bytes = generer_pdf_bulletin(b_data)
                st.download_button(
                    "📄 Télécharger le Bulletin de Notes Officiel (PDF)",
                    data=pdf_b_bytes,
                    file_name=f"Bulletin_{enfant_classe}_{enfant_nom.replace(' ', '_')}_{p_sel_bul.replace(' ', '_')}.pdf",
                    mime="application/pdf",
                    key="dl_pdf_bulletin_parent"
                )
            else:
                st.warning("Aucune période enregistrée pour cette classe.")

        # TAB 3 : EMPLOI DU TEMPS
        with tp_edt:
            st.markdown(f"### 📅 Emploi du Temps Officiel - Classe : {enfant_classe}")
            edt_parent = get_or_create_edt(enfant_classe)
            st.dataframe(edt_parent, use_container_width=True)
            pdf_edt_p = generer_pdf_edt(enfant_classe, edt_parent)
            st.download_button("📥 Télécharger L'Emploi du Temps (PDF)", data=pdf_edt_p, file_name=f"EDT_{enfant_classe}.pdf", mime="application/pdf", key="dl_edt_parent")

        # TAB 4 : CAHIER DE TEXTE
        with tp_ct:
            st.markdown(f"### 📑 Cahier de Texte de la Classe : {enfant_classe}")
            df_ct_p = st.session_state.cahier_textes[st.session_state.cahier_textes["Classe"] == enfant_classe] if "cahier_textes" in st.session_state and not st.session_state.cahier_textes.empty and "Classe" in st.session_state.cahier_textes.columns else pd.DataFrame()
            if not df_ct_p.empty:
                st.dataframe(df_ct_p[["Date", "Matière", "Professeur", "Contenu", "Travail à faire"]], use_container_width=True)
            else:
                st.info("Aucune séance consignée dans le cahier de texte pour l'instant.")

        # TAB 5 : VIE SCOLAIRE
        with tp_vs:
            st.markdown(f"### ⚠️ Bilan Vie Scolaire & Assiduité")
            df_vs_p = st.session_state.viescolaire_db if "viescolaire_db" in st.session_state else pd.DataFrame()
            if not df_vs_p.empty and "Eleve" in df_vs_p.columns:
                sub_vs = df_vs_p[df_vs_p["Eleve"] == enfant_nom]
                if not sub_vs.empty:
                    st.dataframe(sub_vs, use_container_width=True)
                else:
                    st.info("Aucun incident ou remarque particulière de vie scolaire.")
            else:
                st.info("Aucune donnée enregistrée dans la vie scolaire.")

        # TAB 6 : MESSAGES & ÉCHANGES
        with tp_msg:
            st.markdown("### 💬 Échanges & Messages avec l'Établissement")
            with st.form("form_msg_parent", clear_on_submit=True):
                obj_msg = st.text_input("Objet du message")
                body_msg = st.text_area("Votre message à l'administration / professeurs")
                urg_msg = st.checkbox("Message Urgent")
                ...
                if btn_send_msg:
                    if obj_msg and body_msg:
                        ...
                        new_msg = {
                            "ID": msg_id, "Emetteur": f"Parent de {enfant_nom}", "RoleEmetteur": "Parent",
                            "DateEnvoi": str(datetime.now().strftime("%Y-%m-%d %H:%M")), "Classe": enfant_classe,
                            "Objet": obj_msg, "Message": body_msg, "Urgent": urg_msg
                        }
            ...
            df_m_p = st.session_state.messages_parents_db[st.session_state.messages_parents_db["Classe"] == enfant_classe] if "messages_parents_db" in st.session_state and not st.session_state.messages_parents_db.empty and "Classe" in st.session_state.messages_parents_db.columns else pd.DataFrame()
            if not df_m_p.empty:
                for _, r_m in df_m_p.iterrows():
                    st.markdown(
                        f"""
                        <div class="msg-card">
                            <h5 style="margin: 0; color: #4F46E5;"><b>Objet :</b> {r_m.get('Objet', 'Sans objet')}</h5>
                            <p style="margin: 4px 0; font-size: 0.85rem; color: #64748B;"><b>De :</b> {r_m.get('Emetteur', 'N/A')} | <b>Date :</b> {r_m.get('DateEnvoi', 'N/A')}</p>
                            <p style="margin: 8px 0 0 0; color: #1E293B;">{r_m.get('Message', '')}</p>
                        </div>
                        """,
                        unsafe_allow_html=True
                    )
...
# ==========================================
    ...
    st.markdown('<div style="color: #0F172A; font-size: 2.2rem; font-weight: 900;">🔒 Espace Administration & Contrôle d\'Accès</div>', unsafe_allow_html=True)

    if not st.session_state.authenticated_admin:
        with st.form("form_login_admin"):
            adm_email = st.text_input("Identifiant Administrateur (Email)")
            adm_pass = st.text_input("Mot de passe sécurisé", type="password")
            btn_adm = st.form_submit_button("Se Connecter à l'Administration")

            if btn_adm:
                if (adm_email == ADMIN_EMAIL and adm_pass == "cpnm2026") or verifier_mot_de_passe(adm_pass, hacher_mot_de_passe("cpnm2026")):
                    st.session_state.authenticated_admin = True
                    enregistrer_log_action(adm_email, "CONNEXION_ADMIN", "Connexion réussie à l'Espace Administration")
                    st.success("Connexion Administration Réussie !")
                    st.rerun()
                else:
                    st.error("Identifiants Administration incorrects.")
    else:
        ...
        t_classes, t_eleves, t_profs, t_coeffs, t_pers, t_logs = st.tabs([
            "🏫 Gestion des Classes",
            "👨‍🎓 Gestion des Élèves",
            "👨‍🏫 Liste Blanche Professeurs",
            "⚖️ Coefficients & Barèmes",
            "📅 Périodes Scolaires",
        ...
        with t_classes:
            ...
            ed_classes = st.data_editor(st.session_state.classes_db, num_rows="dynamic", use_container_width=True, key="ed_classes_admin")
            if st.button("💾 Enregistrer les Classes", key="save_cls_adm"):
                st.session_state.classes_db = ed_classes
                df_c_save = ed_classes.rename(columns={"Classe": "classe", "Cycle": "cycle", "Professeur Responsable": "professeur_responsable"})[["classe", "cycle", "professeur_responsable"]]
                save_df_to_db(df_c_save, "classes")
                st.success("✅ Classes enregistrées dans Supabase !")

        with t_eleves:
            st.markdown("### 👨‍🎓 Base de Données des Élèves")
            ed_eleves = st.data_editor(st.session_state.eleves_db, num_rows="dynamic", use_container_width=True, key="ed_eleves_admin")
            if st.button("💾 Enregistrer les Élèves", key="save_el_adm"):
                st.session_state.eleves_db = trier_eleves_par_nom(ed_eleves)
                df_e_save = st.session_state.eleves_db.rename(columns={"Nom Complet": "nom_complet", "Prénom": "prenom", "Nom": "nom", "Date de Naissance": "date_de_naissance", "Classe": "classe", "Photo": "photo"})[["nom_complet", "prenom", "nom", "date_de_naissance", "classe", "photo"]]
                save_df_to_db(df_e_save, "eleves")
                st.success("✅ Élèves enregistrés dans Supabase !")

        with t_profs:
            st.markdown("### 👨‍🏫 Liste Blanche & Accréditations Enseignants")
            ed_profs = st.data_editor(st.session_state.prof_credentials, num_rows="dynamic", use_container_width=True, key="ed_profs_admin")
            if st.button("💾 Enregistrer la Liste Blanche Enseignants", key="save_prof_adm"):
                st.session_state.prof_credentials = ed_profs
                synchroniser_listes_blanches()
                df_p_save = ed_profs.rename(columns={"Nom": "nom", "Prénom": "prenom", "Email": "email", "Matière Principale": "matiere_principale", "Classe Attribuée": "classe_attribuee", "Mot de passe": "password"})[["nom", "prenom", "email", "matiere_principale", "classe_attribuee", "password"]]
                save_df_to_db(df_p_save, "prof_white_list")
                st.success("✅ Enseignants sauvegardés dans Supabase !")

        with t_coeffs:
            st.markdown("### ⚖️ Configuration des Coefficients & Barèmes par Classe")
            ed_coeffs = st.data_editor(st.session_state.coefficients_db, num_rows="dynamic", use_container_width=True, key="ed_coeffs_admin")
            if st.button("💾 Enregistrer Coefficients", key="save_coef_adm"):
                st.session_state.coefficients_db = ed_coeffs
                df_co_save = ed_coeffs.rename(columns={"Classe": "classe", "Matière": "matiere", "Coefficient": "coefficient", "Barème": "bareme"})[["classe", "matiere", "coefficient", "bareme"]]
                save_df_to_db(df_co_save, "coefficients")
                st.success("✅ Coefficients enregistrés !")

        with t_pers:
            st.markdown("### 📅 Gestion des Périodes Académiques")
            ed_pers = st.data_editor(st.session_state.periodes_db, num_rows="dynamic", use_container_width=True, key="ed_pers_admin")
            if st.button("💾 Enregistrer Périodes", key="save_per_adm"):
                st.session_state.periodes_db = ed_pers
                df_per_save = ed_pers.rename(columns={"Période": "periode", "Statut": "statut", "Cycle": "cycle"})[["periode", "statut", "cycle"]]
                save_df_to_db(df_per_save, "periodes")
                st.success("✅ Périodes enregistrées !")

        with t_logs:
            st.markdown("### 📜 Journal d'Audit & Actions Utilisateurs")
            st.dataframe(st.session_state.audit_logs_db, use_container_width=True)

# ==========================================
# 9. RAPPORTS GLOBAUX & EXPORTS PDF XXL
# ==========================================
elif st.session_state.espace_actif == "🏫 Administration XXL & Rapports":
    st.markdown('<div style="color: #0F172A; font-size: 2.2rem; font-weight: 900;">🏫 Rapports Globaux, Documents Officiels & IA Assistant</div>', unsafe_allow_html=True)

    tr_buls, tr_lists, tr_abs, tr_ia = st.tabs([
        "📄 Bulletins de Classe (ZIP Batch)",
        "📋 Fiches Officielle de Classe (PDF)",
        "⚠️ Registre Global des Absences (PDF)",
        "🤖 Assistant Virtuel Pédagogique IA"
    ])

    with tr_buls:
        st.markdown("### 📄 Génération Groupée des Bulletins de Classe")
        col_g1, col_g2 = st.columns(2)
        with col_g1:
            cls_gen = st.selectbox("Sélectionner la Classe", st.session_state.classes_db["Classe"].tolist() if not st.session_state.classes_db.empty else ["6ème A"], key="cls_gen_zip")
        with col_g2:
            per_gen = st.selectbox("Sélectionner la Période", obtenir_periodes_pour_classe(cls_gen), key="per_gen_zip")

        if st.button("📦 Générer l'Archive ZIP de la Classe", key="btn_zip"):
            zip_data = generer_zip_bulletins_classe(cls_gen, per_gen)
            st.download_button(f"📥 Télécharger ZIP Bulletins - {cls_gen}", data=zip_data, file_name=f"Bulletins_{cls_gen}_{per_gen}.zip", mime="application/zip")

    with tr_lists:
        st.markdown("### 📋 Imprimer Fiche Officielle d'Élèves par Classe")
        cls_fiche = st.selectbox("Sélectionner la Classe pour Impression", st.session_state.classes_db["Classe"].tolist() if not st.session_state.classes_db.empty else ["6ème A"], key="cls_fiche_pdf")
        pdf_f = generer_pdf_liste_eleves_classe(cls_fiche)
        st.download_button(f"📄 Télécharger Liste Officielle (PDF) - {cls_fiche}", data=pdf_f, file_name=f"Fiche_Classe_{cls_fiche}.pdf", mime="application/pdf")

    with tr_abs:
        st.markdown("### ⚠️ Registre & Exportation des Absences")
        cls_abs = st.selectbox("Filtrer par Classe", ["Toutes"] + (st.session_state.classes_db["Classe"].tolist() if not st.session_state.classes_db.empty else []), key="cls_abs_pdf")
        pdf_a = generer_pdf_liste_absences(cls_abs)
        st.download_button("📄 Télécharger le Registre des Absences (PDF)", data=pdf_a, file_name=f"Registre_Absences_{cls_abs}.pdf", mime="application/pdf")

    with tr_ia:
        st.markdown("### 🤖 Assistant Virtuel Pédagogique National")
        q_user = st.text_input("Posez votre question à l'Assistant IA Pédagogique")
        if q_user:
            rep_ia = assistant_ia_repondre(q_user)
            st.info(f"<b>Réponse de l'Assistant :</b><br>{rep_ia}")
