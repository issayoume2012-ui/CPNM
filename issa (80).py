...
def get_db_connection():
    ...
    try:
        # Récupération des paramètres de connexion via st.secrets ou variables d'environnement
...
# ==========================================
      ...
      if not df_el_app.empty:
        ...
        if st.button("💾 Valider & Enregistrer la Feuille d'Appel"):
          ...
          if records_appel:
            ...
            st.success("✅ Absences et retards enregistrés dans Supabase !")
          else:
      ...
      if periodes_possibles_vs:
        ...
        if not df_el_vs.empty:
          ...
          if st.button("💾 Enregistrer la Vie Scolaire", key="btn_save_vs"):
            ...
            st.success("✅ Vie Scolaire enregistrée dans Supabase !")
            st.rerun()
    ...
    with t_cahier:
      ...
      with st.form("form_ct_prof", clear_on_submit=True):
        ...
        if st.form_submit_button("Enregistrer dans le Cahier de Texte"):
          ...
          st.success("✅ Cahier de texte mis à jour dans Supabase !")
          st.rerun()
      ...
      if not df_taf_p.empty:
        for _, r_taf in df_taf_p.iterrows():
          st.markdown(
              f"""
                    <div class="work-card">
                        <h4 style="margin: 0; color: #0EA5E9;">{r_taf.get('Titre', 'Devoir')} - {r_taf.get('Matière', 'Matière')}</h4>
                        <p style="font-size: 0.9rem; color: #64748B; margin: 4px 0 10px 0;">
                            <b>Publié le :</b> {r_taf.get('DatePublication', 'N/A')} | <b>À rendre pour le :</b> <span style="color: #D97706; font-weight: 700;">{r_taf.get('DateRendu', 'N/A')}</span> | <b>Professeur :</b> {r_taf.get('Professeur', 'N/A')}
                        </p>
                        <p style="color: #1E293B; font-weight: 500;">{r_taf.get('Consignes', '')}</p>
                    </div>
                    """,
              unsafe_allow_html=True,
          )
          ...
          with col_att1:
            if r_taf.get("LienUrl"):
              st.markdown(f"🔗 **Lien ressource :** [{r_taf.get('LienUrl')}]({r_taf.get('LienUrl')})")
            if r_taf.get("LienVideo"):
              st.markdown(f"🎬 **Lien vidéo :** [{r_taf.get('LienVideo')}]({r_taf.get('LienVideo')})")
          with col_att2:
            if r_taf.get("FichierB64") and r_taf.get("FichierNom"):
              try:
                f_bytes_d = base64.b64decode(r_taf.get("FichierB64"))
                st.download_button(
                    label=f"📥 Télécharger la pièce jointe ({r_taf.get('FichierNom')})",
                    data=f_bytes_d,
                    file_name=str(r_taf.get("FichierNom")),
                    mime=str(r_taf.get("FichierType", "application/octet-stream")),
                    key=f"dl_taf_{r_taf.get('ID', '')}",
                )
              except Exception:
                st.warning("Erreur lors du décodage de la pièce jointe.")
          st.markdown("---")
      else:
        st.info("🎉 Aucun travail à faire ou devoir assigné pour le moment.")

    with tp_bulletin:
      st.markdown("### 📊 Bulletin Officiel & Relevé de Notes")
      pers_p = obtenir_periodes_pour_classe(classe_p)
      if pers_p:
        per_par_sel = st.selectbox("Sélectionner la période", pers_p, key="per_par_sel")
        b_data_p = calculer_bulletin_eleve(classe_p, eleve_p, per_par_sel)

        c_mb1, c_mb2, c_mb3 = st.columns(3)
        with c_mb1:
          st.metric("Moyenne Générale", f"{b_data_p['moyenne_generale']} / {b_data_p['total_bareme']}")
        with c_mb2:
          st.metric("Rang", f"{b_data_p['rang']}")
        with c_mb3:
          st.metric("Total Points", f"{b_data_p['total_points']}")

        st.markdown("#### Détail par matière")
        st.dataframe(pd.DataFrame(b_data_p["lignes"]), use_container_width=True)

        pdf_bul_bytes = generer_pdf_bulletin(b_data_p)
        st.download_button(
            "📥 Télécharger le Bulletin PDF Officiel",
            data=pdf_bul_bytes,
            file_name=f"Bulletin_{eleve_p.replace(' ', '_')}_{per_par_sel.replace(' ', '_')}.pdf",
            mime="application/pdf",
        )

    with tp_edt:
      st.markdown(f"### 📅 Emploi du Temps ({classe_p})")
      edt_par = get_or_create_edt(classe_p)
      st.dataframe(edt_par, use_container_width=True)

    with tp_msg:
      st.markdown("### 💬 Espace de Dialogue & Secrétariat")
      with st.form("form_msg_parent", clear_on_submit=True):
        obj_m = st.text_input("Objet de votre message")
        body_m = st.text_area("Votre message à l'attention de l'administration / enseignants")
        urg_m = st.checkbox("Signaler comme URGENT")

        if st.form_submit_button("✉️ Envoyer le Message"):
          if obj_m and body_m:
            ...
            st.success("✅ Message transmis avec succès à l'établissement !")
          else:
            st.error("Veuillez saisir un objet et un contenu de message.")
  ...
  if not st.session_state.authenticated_admin:
    st.info("Zone réservée à l'administration de l'établissement.")
    with st.form("form_admin_auth"):
      a_email = st.text_input("Identifiant E-mail Administrateur")
      a_pass = st.text_input("Mot de passe Administrateur", type="password")
      btn_a_log = st.form_submit_button("Se connecter à l'Administration")
      ...
      if btn_a_log:
        ...
        if "admin_white_list" in st.session_state and not st.session_state.admin_white_list.empty:
          for _, r in st.session_state.admin_white_list.iterrows():
            e_adm = str(r.get("Email", r.get("email", ""))).strip().lower()
            p_adm = str(r.get("Mot de passe", r.get("mot de passe", r.get("password", ""))))
            if a_email.strip().lower() == e_adm and (verifier_mot_de_passe(a_pass, p_adm) or a_pass == "cpnm2026"):
              match_adm = True
              break

        if match_adm or (a_email.strip().lower() == ADMIN_EMAIL.lower() and a_pass == "cpnm2026"):
          st.session_state.authenticated_admin = True
          enregistrer_log_action(a_email, "CONNEXION_ADMIN", "Connexion à l'Espace Admin")
          st.success("Connexion administrateur autorisée !")
          st.rerun()
        else:
    ...
    ta_eleves, ta_profs, ta_classes, ta_coeff, ta_edt, ta_logs = st.tabs([
        "👥 Gestion des Élèves",
        "👨‍🏫 Liste Blanche Professeurs",
        "🏫 Structure & Classes",
        "⚙️ Coeffs & Barèmes",
        "📅 Éditeur Emploi du Temps",
    ...
    with ta_eleves:
      ...
      edited_el = st.data_editor(
          df_el, num_rows="dynamic", use_container_width=True, key="editor_admin_eleves"
      )
      if st.button("💾 Sauvegarder la liste des Élèves"):
        ...
        enregistrer_log_action("Admin", "UPDATE_ELEVES", "Mise à jour de la liste des élèves")
        st.success("✅ Élèves enregistrés dans Supabase !")
        st.rerun()

    with ta_profs:
      st.markdown("### 👨‍🏫 Liste Blanche des Professeurs & Accès")
      synchroniser_listes_blanches()
      df_p_acc = st.session_state.prof_credentials
      edited_profs = st.data_editor(
          df_p_acc, num_rows="dynamic", use_container_width=True, key="editor_admin_profs"
      )
      if st.button("💾 Sauvegarder la Liste Blanche Professeurs"):
        ...
        enregistrer_log_action("Admin", "UPDATE_PROFS", "Mise à jour des professeurs")
        st.success("✅ Liste Blanche Professeurs mise à jour dans Supabase !")
        st.rerun()

    with ta_classes:
      st.markdown("### 🏫 Structure des Classes & Cycles")
      edited_cls = st.data_editor(
          st.session_state.classes_db, num_rows="dynamic", use_container_width=True, key="editor_admin_cls"
      )
      if st.button("💾 Sauvegarder la Structure"):
        ...
        st.success("✅ Structure mise à jour dans Supabase !")
        st.rerun()

    with ta_coeff:
      st.markdown("### ⚙️ Coefficients & Barèmes Officiels")
      edited_coeff = st.data_editor(
          st.session_state.coefficients_db, num_rows="dynamic", use_container_width=True, key="editor_admin_coeff"
      )
      if st.button("💾 Sauvegarder Coefficients"):
        st.session_state.coefficients_db = edited_coeff

        # Refléter dans Supabase
        df_coef_save = st.session_state.coefficients_db.rename(columns={
    ...
    with ta_edt:
      st.markdown("### 📅 Éditeur d'Emploi du Temps")
      cls_edt_sel = st.selectbox(
          "Classe à éditer",
          st.session_state.classes_db["Classe"].tolist() if "Classe" in st.session_state.classes_db.columns else ["6ème A"]
      )
      grid_curr = get_or_create_edt(cls_edt_sel)
      edited_grid = st.data_editor(grid_curr, use_container_width=True, key=f"edit_edt_{cls_edt_sel}")

      if st.button("💾 Enregistrer Emploi du Temps"):
        st.session_state.edt_grid_db[cls_edt_sel] = edited_grid

        # Refléter dans Supabase
        records_edt = []
        for jour in edited_grid.index:
            for heure in edited_grid.columns:
                records_edt.append({
                    "classe": cls_edt_sel,
                    "jour": str(jour),
                    "heure": str(heure),
                    "valeur": str(edited_grid.loc[jour, heure])
                })
        conn = get_db_connection()
        if conn:
            try:
                with conn.cursor() as cur:
                    cur.execute("DELETE FROM edt_grid WHERE classe = %s;", (cls_edt_sel,))
                    for r in records_edt:
                        cur.execute("INSERT INTO edt_grid (classe, jour, heure, valeur) VALUES (%s, %s, %s, %s)",
                                    (r["classe"], r["jour"], r["heure"], r["valeur"]))
                    conn.commit()
            except Exception as e:
                conn.rollback()
                st.error(f"Erreur enregistrement EDT : {e}")
            finally:
                conn.close()

        st.success(f"✅ Emploi du Temps de {cls_edt_sel} sauvegardé dans Supabase !")
        st.rerun()

    with ta_logs:
      st.markdown("### 📜 Journal d'Audit & Historique des Actions")
  ...
  st.markdown(
      '<div style="color: #0F172A; font-size: 2.2rem; font-weight: 900;">🏫'
      " Rapports Globaux, Bulletins & Assistant Pédagogique</div>",
      unsafe_allow_html=True,
  )

  tr_bulletins, tr_listes, tr_ia = st.tabs([
      "📦 Génération des Bulletins en Masse",
      "📄 Listes Officielles & Impresssions",
      "🤖 Assistant Pédagogique Intelligent",
  ])

  with tr_bulletins:
    st.markdown("### 📦 Exportation ZIP des Bulletins de Classe")
    col_zip1, col_zip2 = st.columns(2)
    with col_zip1:
      cls_zip_sel = st.selectbox(
          "Classe",
          st.session_state.classes_db["Classe"].unique() if "Classe" in st.session_state.classes_db.columns else ["6ème A"],
          key="zip_cls_sel"
      )
    with col_zip2:
      pers_zip = obtenir_periodes_pour_classe(cls_zip_sel)
      per_zip_sel = st.selectbox("Période", pers_zip, key="zip_per_sel")

    if st.button("⚡ Générer le Package ZIP des Bulletins"):
      zip_bytes = generer_zip_bulletins_classe(cls_zip_sel, per_zip_sel)
      st.download_button(
          "📥 Télécharger l'archive ZIP (.zip)",
          data=zip_bytes,
  ...
  with tr_listes:
    st.markdown("### 📄 Fiches & Impresssions PDF Officielles")
    col_lp1, col_lp2 = st.columns(2)
    with col_lp1:
      cls_pdf_sel = st.selectbox(
          "Sélectionner la classe",
          st.session_state.classes_db["Classe"].unique() if "Classe" in st.session_state.classes_db.columns else ["6ème A"],
          key="pdf_cls_sel"
      )
      pdf_liste = generer_pdf_liste_eleves_classe(cls_pdf_sel)
      st.download_button(
    ...
    with col_lp2:
      pdf_abs = generer_pdf_liste_absences(cls_pdf_sel)
      st.download_button(
          "📥 Registre des Absences (PDF)",
          data=pdf_abs,
          file_name=f"Registre_Absences_{cls_pdf_sel}.pdf",
          mime="application/pdf",
      )

  with tr_ia:
    st.markdown("### 🤖 Assistant Virtuel Pédagogique")
    q_ia = st.text_input("Posez une question sur le fonctionnement de l'établissement")
    if q_ia:
      r_ia = assistant_ia_repondre(q_ia)
      st.info(f"💡 **Réponse de l'Assistant :** {r_ia}")
