...
def init_db():
    ...
    try:
        with conn.cursor() as cur:
            # Table Audit Logs
            cur.execute("CREATE TABLE IF NOT EXISTS audit_logs (id SERIAL PRIMARY KEY, horodatage VARCHAR(50), acteur VARCHAR(255), action VARCHAR(255), details TEXT);")
            # Table Admin
            cur.execute("CREATE TABLE IF NOT EXISTS admin_white_list (id SERIAL PRIMARY KEY, email VARCHAR(255) UNIQUE NOT NULL, nom VARCHAR(255), prenom VARCHAR(255), password VARCHAR(255), niveau_acces VARCHAR(255));")
            # Table Prof (ISS-80 Logic Integration)
            cur.execute("""
...
def synchroniser_listes_blanches():
    """Maintient la cohérence entre la base de données et la session pour ISSA-80."""
    conn = get_db_connection()
    if conn:
        try:
            query = 'SELECT nom AS "Nom", prenom AS "Prénom", email AS "Email", matiere_principale AS "Matière Principale", classe_attribuee AS "Classe Attribuée", password AS "Mot de passe" FROM prof_white_list'
            df_prof_db = pd.read_sql(query, conn)
...
# ==========================================
# ==========================================
# 4. NAVIGATION PRINCIPALE (Harmonisée)
# ==========================================

# Définition des noms d'espaces pour correspondre à vos boutons et votre logique
noms_espaces = [
    "🏠 Accueil", 
    "🛡️ Espace Administration (Sécurisé)", 
    "👨‍🏫 Espace Professeurs / Maîtres", 
    "👨‍👩‍👧 Espace Parents / Élèves",
    "🏫 Administration XXL & Rapports"
]

# Affichage de la navigation
st.session_state.espace_actif = st.radio("Navigation", noms_espaces, horizontal=True, label_visibility="collapsed")

# ==========================================
# 5. ACCUEIL ET REDIRECTION SÉLECTIVE
# ==========================================
if st.session_state.espace_actif == "🏠 Accueil":
    st.markdown("""<div style="text-align: center; padding: 15px 0 35px 0;">
    ...
    with c3:
        ...
        if st.button("Accéder Admin", key="btn_ad"):
            st.session_state.espace_actif = "🛡️ Espace Administration (Sécurisé)"
            st.rerun()
    with c4:
        # Votre contenu HTML...
