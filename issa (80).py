...
SUPABASE_URL = "https://dzxotavktglasrcpygwx.supabase.co"
...
def charger_table_supabase(nom_table, colonnes_defaut):
    if supabase is None:
...
# --- INITIALISATION DU SESSION STATE & CHARGEMENT SUPABASE ---
if "messages_parents_db" not in st.session_state:
    df_msg = charger_table_supabase("messages_parents_db", ["ID", "Emetteur", "RoleEmetteur", "DateEnvoi", "Classe", "Objet", "Message", "Urgent"])
    if df_msg.empty:
        df_msg = pd.DataFrame(columns=["ID", "Emetteur", "RoleEmetteur", "DateEnvoi", "Classe", "Objet", "Message", "Urgent"])
...
if "parents_white_list" not in st.session_state:
    df_pw = charger_table_supabase("parents_white_list", ["Téléphone", "Prénom Élève", "Nom Élève", "Classe"])
    if df_pw.empty:
        df_pw = pd.DataFrame([
            {
                "Téléphone": "771234567",
    ...
    st.session_state.parents_white_list = df_pw

if "eleves_db" not in st.session_state:
    df_el = charger_table_supabase("eleves_db", ["Nom Complet", "Nom", "Prénom", "Classe"])
    if df_el.empty:
        df_el = pd.DataFrame([
            {
                "Nom Complet": "Issa",
    ...
    st.session_state.eleves_db = df_el

if "travail_a_faire_db" not in st.session_state:
    st.session_state.travail_a_faire_db = charger_table_supabase("travail_a_faire_db", ["ID", "Titre", "Matière", "Professeur", "DateRendu", "Consignes", "Classe", "LienUrl", "LienVideo", "FichierB64", "FichierNom"])

if "notes_db" not in st.session_state:
    st.session_state.notes_db = charger_table_supabase("notes_db", ["Classe", "Periode", "Eleve", "Matière", "Devoir1", "Devoir2", "Composition"])

if "viescolaire_db" not in st.session_state:
    st.session_state.viescolaire_db = charger_table_supabase("viescolaire_db", ["Classe", "Periode", "Eleve", "AbsencesJustifiees", "AbsencesNonJustifiees", "Retards", "HeuresPerdues", "Observations", "DecisionConseil"])

if "coefficients_db" not in st.session_state:
    st.session_state.coefficients_db = charger_table_supabase("coefficients_db", ["Classe", "Matière", "Coefficient", "Barème"])

if "matieres_def" not in st.session_state:
    st.session_state.matieres_def = charger_table_supabase("matieres_def", ["Matière", "Cycle", "Coefficient", "Barème"])

if "periodes_db" not in st.session_state:
    st.session_state.periodes_db = charger_table_supabase("periodes_db", ["Cycle", "Période"])

if "classes_db" not in st.session_state:
    st.session_state.classes_db = charger_table_supabase("classes_db", ["Classe", "Cycle"])
                ...
                if st.form_submit_button("Envoyer à l'Administration"):
                    if objet_m and msg_m:
                        ...
                        if supabase is not None:
                            try:
                                supabase.table("messages_parents_db").insert(nouveau_msg).execute()
                            except Exception as e:
                                st.warning(f"Message enregistré localement mais échec de la synchronisation Supabase : {e}")
